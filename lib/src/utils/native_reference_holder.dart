...
/// 
/// This class stores tracking addresses in a cross-isolate, persistant way (via a tmp file keyed by PID)
...
class NativeReferenceHolder {
  ...
  NativeReferenceHolder._() {
    ...
    _file = File('${Directory.systemTemp.path}${Platform.pathSeparator}mpv_audio_kit_refs_$pid.txt');
  }

  /// Called at the very beginning of the app (e.g. `MpvAudioKit.ensureInitialized()`)
  void ensureInitialized(void Function(List<Pointer<MpvHandle>>) onOrphanFound) {
    if (!_isDebug) return;
    ...
    try {
      ...
      debugPrint('mpv_audio_kit: NativeReferenceHolder initialization failed: $e');
    } finally {
