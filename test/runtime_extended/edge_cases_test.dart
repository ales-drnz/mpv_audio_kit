...
void main() {
  runtimeSuite(() {
    group('Edge cases — exotic fixtures and boundary inputs', () {
      ...
      test(
        '88.2 kHz sample rate fixture decodes with correct audio-params',
        () async {
          final exoticPath =
          ...
          expect(
            ...
            88200,
            reason:
                'fixture is encoded at 88.2 kHz; '
      ...
      test(
        'rapid sequential setVolume calls converge to the last value',
        () async {
      ...
      test('concurrent open() calls — last one wins (replace aborts the '
        ...
        expect(
          ...
          88200,
          reason:
              'open(B) was issued after open(A) without await between '
      ...
      test('setVolume above 100 retains the optimistic value (mpv clamps to '
        ...
        expect(
          ...
          150.0,
          reason:
              'wrapper writes the requested value optimistically; mpv '
      ...
      test('setRate above 100.0 throws MpvException (mpv hard-rejects the '
      ...
      test('setRate below 0.01 throws MpvException (mpv hard-rejects sub-0.01 '
      ...
      test(
        'setDemuxer preserves sub-MiB precision (no MiB rounding)',
        () async {
          ...
          await player.setDemuxer(
            player.state.demuxer.copyWith(maxBytes: bytes),
          );
